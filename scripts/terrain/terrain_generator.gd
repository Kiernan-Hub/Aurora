...
# The ice colours actually in use, as opposed to the FILL_GRADIENT_* constants, which
...
# The ice pattern, as the two ends of a crossfade plus where it currently sits. Same headless
# argument as the two tints above: apply_ice_palette() never runs under --headless, so every
...
# fill. See docs/development/visuals.md, "Terrain fill shading".
# Now multipliers against ICE_TERRAIN_TEXTURE (Polygon2D samples texture * vertex_color
# per pixel) rather than the flat colour itself -- near-white at the surface so the
# texture's own ice colour and detail show through untouched, dimming toward a neutral
# grey (not a saturated tint, which would fight the texture's own hue) at depth.
const FILL_GRADIENT_TOP_TINT: Color = Color(1.0, 1.0, 1.0, 1.0)
const FILL_GRADIENT_BOTTOM_TINT: Color = Color(0.55, 0.58, 0.66, 1.0)
...
# --- Surface rim: REMOVED 2026-08-08 -----------------------------------------------
# Two stacked Line2D per ground run used to trace the surface: a 22px low-alpha glow under
# a 5px near-opaque core, faking a bloom highlight without a shader. Removed because the
# depth tile now carries a real snow band in its top rows, which is both softer and
# physically sensible (snow settled on the lip) where the Line2D read as a drawn outline
...
# unlifted darks multiply every biome tint down to black. The script also cross-fades the
# horizontal wrap; it deliberately does NOT mirror, which is what stamped the visible
# diamond artifact into the tile this replaced.
#
# UV.x MUST be world_x, not the chunk-local x that `polygon` itself is built in. Chunk
# geometry is deliberately chunk-local (build_chunk_surface's local_x), but a texture
# UV'd in that same local space would restart its phase at every chunk boundary --
# exactly the seam-every-segment look this pass exists to avoid. world_x has no such
# reset, so consecutive chunks tile the texture continuously across the boundary.
# (world_x is allowed to grow unbounded over a long session -- see main.gd's "X is never
...
# repeat down to ~1.6s at MAX_SPEED instead of ~0.5s.
const ICE_TILE_WORLD_WIDTH: float = 1200.0
# Keeps the deepest sample a texel clear of the bottom edge. texture_repeat applies to BOTH
# axes in Godot, so a V landing exactly on the texture height wraps around to row 0 -- the
# bright snow band -- and paints a white line straight across the fill at the gradient stop.
const ICE_TILE_V_INSET: float = 1.0
# The tile's own darkest value. MUST MATCH OUTPUT_FLOOR in scripts/tools/build_ice_texture.py.
# The deep fill below the band carries no texture, so it has to multiply its tint by this by
# hand or there is a visible brightness step exactly where the band ends.
const ICE_TILE_DEPTH_FLOOR: float = 0.38
# How far below the surface the tile's full ramp is spread. NOT FILL_GRADIENT_DEPTH (560):
# at normal camera height only ~300px of fill is ever on screen, so a 560px band showed only
# the tile's pale top half and the ice read as grey concrete -- it never reached its deep
...
func spawn_chunk(chunk_index: int) -> void:
	...
	# No colour call here any more: build_chunk_fill already reads the live ice tints below,
	# so a chunk is born in the current biome. repaint_chunk() (the old apply_chunk_color,
	# repointed) exists for the chunks that were ALREADY on screen when the biome moved.
	add_child(chunk)
...
func build_chunk_surface(chunk: StaticBody2D, chunk_index: int) -> void:
	...
	build_chunk_fill(chunk, terrain_fill, surface_points, visual_sample_world_xs, chunk_start_x)
...
func build_chunk_fill(chunk: StaticBody2D, terrain_fill: Polygon2D, surface_points: PackedVector2Array, surface_world_xs: Array[float], chunk_start_x: float) -> void:
	...
	# Bands last, so every one of them draws over every fill -- including the extra fills a
...
# deep fill polygon (2026-08-08). This is the fix for the funnel/tunnel warping and the
# hard per-chunk seams that the first attempt at depth mapping produced.
#
# WHY THE DEEP FILL COULD NOT CARRY THIS. close_fill_run() gives the fill ~64 vertices
# along the top and exactly FOUR at the bottom, so Polygon2D triangulates it as long skinny
# triangles fanning out from those two bottom corners. UV interpolates LINEARLY per
# triangle, which is exact only when the UV map is affine in position. World-space UVs (the
# old mapping) are affine, so any triangulation renders identically -- that is why it never
# distorted. "Depth below a curved surface" is NOT affine, so fanning it across those
# triangles smeared the texture into a funnel, and because each chunk fans from its own two
# corners the funnel restarted at every chunk boundary as a hard seam.
#
# So the band is built as an explicit QUAD STRIP: one quad per surface sample, top edge on
# the surface, bottom edge FILL_GRADIENT_DEPTH below it. Polygon2D.polygons is set to those
# quads by index, so Godot does no triangulation guessing at all, and V is exact at every
# vertex instead of interpolated across a whole chunk. Each quad is small and near-
# rectangular, so linear interpolation inside it is visually exact.
#
# Continuity across chunks is automatic and needs no bookkeeping: a chunk's last surface
...
func build_ice_band(chunk: StaticBody2D, run_points: PackedVector2Array, chunk_start_x: float, run_index: int) -> void:
	...
	# Read off the BASE tile and used for BOTH bands, which is only correct because every tile
	# is built at the same size -- build_ice_texture.py's OUTPUT_SIZE, asserted per palette by
	# biome_schedule_check.gd's EXPECTED_ICE_TILE_SIZE. A differently sized variant would give
	# the overlay UVs meant for the other tile and slide the two patterns against each other
	# mid-dissolve. That gate is what makes sharing this safe; don't drop it.
	var texture_size: Vector2 = ice_band_texture.get_size()
	# Polygon2D UVs are in texture PIXELS, not normalised, so one repeat spans texture_size.x.
	var horizontal_scale: float = texture_size.x / ICE_TILE_WORLD_WIDTH
	var world_x_offset: float = chunk_start_x + (chunk_width * 0.5)
	var deep_v: float = texture_size.y - ICE_TILE_V_INSET

	var band_points: PackedVector2Array = PackedVector2Array()
	var band_uv: PackedVector2Array = PackedVector2Array()
	band_points.resize(sample_count * 2)
	band_uv.resize(sample_count * 2)

	for sample_index: int in range(sample_count):
		var surface_point: Vector2 = run_points[sample_index]
		var texture_u: float = (surface_point.x + world_x_offset) * horizontal_scale
		band_points[sample_index] = surface_point
		band_uv[sample_index] = Vector2(texture_u, 0.0)
		# The lower row follows the surface rather than sitting on a flat line, which is
		# what makes V mean the same thing at every x no matter what the terrain does.
		band_points[sample_count + sample_index] = Vector2(surface_point.x, surface_point.y + ICE_BAND_DEPTH)
		band_uv[sample_count + sample_index] = Vector2(texture_u, deep_v)
...
func build_fill_vertex_colors(run_points: PackedVector2Array) -> PackedColorArray:
	...
	# FLAT, every vertex. The ice band covers the top ICE_BAND_DEPTH of this polygon and is
	# the only part with structure; everything below simply has to continue the band's
	# bottom row without a step. A gradient here would only reintroduce one, because the
	# band now ends shallower than this polygon's own gradient stop.
	var deep_color: Color = get_deep_fill_color()
	for vertex_index: int in range(vertex_colors.size()):
		vertex_colors[vertex_index] = deep_color
...
# FILL_GRADIENT_* constants it has always had and its terrain pixels are unchanged.
#
...
# The two tiles arrive as the biome pair's OWN ice_texture values rather than off `palette`,
# because a blended palette cannot express a crossfade between two textures -- see
# BiomePalette.ice_texture. ice_weight is the ice channel's weight: 0 at the start of a
...
# Repaints every chunk currently on screen when the biome moves. New chunks pick the tints
# up at build time; this is only for the ones already spawned, and without it the ice would
...
func repaint_chunk(chunk: Node2D) -> void:
	...
	for child: Node in chunk.get_children():
		...
		for vertex_index: int in range(vertex_colors.size()):
			vertex_colors[vertex_index] = deep_color
		# Reassigned as a whole array: PackedColorArray is a value type here, so mutating
		# the local copy above does not touch the node until this line.
