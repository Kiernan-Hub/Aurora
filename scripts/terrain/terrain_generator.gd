...
# --- Surface rim (2026-08-07) ------------------------------------------------------
# Two stacked Line2D per ground run, traced along the same surface_points the fill and
# collision already use, so the rim can never drift off the visible edge. A wide,
# low-alpha "glow" line under a thin, near-opaque "core" line fakes a bloom highlight
# without a shader (project has none -- see visuals.md). Built once per chunk spawn,
# same as the fill; not per-frame work.
const RIM_CORE_COLOR: Color = Color(0.97, 0.99, 1.0, 1.0)
const RIM_CORE_WIDTH: float = 5.0
const RIM_GLOW_COLOR: Color = Color(0.85, 0.95, 1.0, 0.35)
const RIM_GLOW_WIDTH: float = 22.0
# --- Fill texture (2026-08-07) ------------------------------------------------------
# Real art in place of the flat vertex-colour gradient: a 1200x520 seamless tile cut from
# a reference ice-skating illustration (assets/textures/terrain/ice_terrain_one.png,
# generated by the project owner, cropped to a crack-free/highlight-free patch of ice body
# and made tileable by 2x2 mirroring -- see the file's provenance note in visuals.md
# before replacing it).
#
...
const ICE_TERRAIN_TEXTURE: Texture2D = preload("res://assets/textures/terrain/ice_terrain_one.png")
...
func build_chunk_fill(chunk: StaticBody2D, terrain_fill: Polygon2D, surface_points: PackedVector2Array, surface_world_xs: Array[float], chunk_start_x: float) -> void:
	...
	apply_fill_texture(terrain_fill, ground_runs[0], fill_bottom_y, chunk_start_x)
	add_surface_rim(chunk, ground_runs[0], 0)
	for run_index: int in range(1, ground_runs.size()):
		...
		apply_fill_texture(extra_fill, ground_runs[run_index], fill_bottom_y, chunk_start_x)
		chunk.add_child(extra_fill)
		add_surface_rim(chunk, ground_runs[run_index], run_index)


# UV in world space (see ICE_TERRAIN_TEXTURE's comment for why), mirroring close_fill_run's
# geometry point-for-point so the texture never stretches independently of the fill shape
# it's mapped onto.
func apply_fill_texture(fill: Polygon2D, run_points: PackedVector2Array, fill_bottom_y: float, chunk_start_x: float) -> void:
	fill.texture = ICE_TERRAIN_TEXTURE
	fill.texture_repeat = CanvasItem.TEXTURE_REPEAT_ENABLED
	var local_uv: PackedVector2Array = close_fill_run(run_points, fill_bottom_y)
	var world_x_offset: float = chunk_start_x + (chunk_width * 0.5)
	var world_uv: PackedVector2Array = PackedVector2Array()
	world_uv.resize(local_uv.size())
	for uv_index: int in range(local_uv.size()):
		world_uv[uv_index] = Vector2(local_uv[uv_index].x + world_x_offset, local_uv[uv_index].y)
	fill.uv = world_uv


# One glow + one core Line2D per ground run, traced along the same points as the run's
# fill polygon. Glow added first so the core draws on top of it; both draw on top of
# TerrainFill because Polygon2D children are added before these in the tree.
func add_surface_rim(chunk: StaticBody2D, run_points: PackedVector2Array, run_index: int) -> void:
