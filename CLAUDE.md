...
| 12 | **Aurora borealis** | The game's namesake. **Phases 1+2a written 2026-09-07, NOT YET RUN — needs a project import (new `class_name`), `check.sh`, then windowed `sky_layer_check`.** ~60 min cumulative playtime, **night only** (blended `star_density` ≥ 0.8), one blend ramp, one per run. Three code-built curtains in `SkyBackdrop`, **additive via `CanvasItemMaterial`, not a third shader**. **It must never arm terrain** — `arm_lake()` is the sole writer of `get_terrain_height`'s one runtime input. `aurora_borealis.md` |
