...
| 12 | **Aurora borealis** | PHASED PLAN, not started — the game's namesake. Rare sky-only spectacle, sibling of the frozen lake but rarer (~60 min cumulative playtime vs. 20), purely cosmetic, one blend ramp. **It must never arm terrain** — `arm_lake()` is the sole writer of `get_terrain_height`'s one runtime input. Four owner decisions block Phase 1. `aurora_borealis.md` |
