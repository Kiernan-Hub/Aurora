...
| 12 | **Aurora borealis** | **IN PROGRESS, phase 1 of 8** on `claude/aurora-borealis-design-1rpnt8`. The namesake feature. **`aurora_borealis.md` is a finished plan, not a sketch — read it before any aurora code and don't re-derive its numbers** (three review rounds; every wrong one came from reasoning off a plausible constant instead of reading it). Every **30 min** cumulative playtime, **dark biomes only**, ~60s of sky curtains on a third shader plus a background wash at `CanvasLayer -45`, everything on one `get_aurora_blend()`. **NOT purely cosmetic**: a write-once/write-ahead *calm band* deletes chasms and suppresses obstacles so the player can look up — the hazards are removed, `die()` is never disabled. `HANDOFF.md` has the phase state |
