...
| 12 | **Aurora borealis** | The namesake feature, **IN PROGRESS**. Every **30 min** cumulative playtime, one per run, only when the whole ~61s maximum-travel window is night (`star_density` ≥ 0.8). Built and pending owner/device review: directional curtain sky; write-ahead protected flat with safe entry/recovery, spawn/powerup policy and lake arbitration; background/ice light; grounded blade glow; bounded snow crest; sparse behind-gameplay wisps; restrained camera zoom/framing; a brief wing apparition with a bounded 96px guided crest flight; a dedicated ambient bed; and the once-only completion achievement. Existing visible objects are never deleted, while coins and non-movement powerups continue. Normal jumping resumes after the flight. `aurora_borealis.md` |
