...
| 12 | **Aurora borealis** | **BUILT; pending owner/device acceptance and shipping-default restoration.** Every **30 min** cumulative playtime, one per run, only past **130s into a run** (the flat is cut at `MAX_SPEED` but ends on a 61s clock, so a still-accelerating player leaves dead flat behind) and only when the whole 61s window is night (`star_density` ≥ 0.8). One director clock owns: directional curtains; write-ahead protected flat with safe entry/recovery, spawn/powerup policy and lake arbitration; background/ice light; grounded blade glow; bounded snow crest; rear wisps; restrained camera zoom/framing; six wings plus one 96px guided crest flight; a dedicated Music-bus ambient bed; and the once-only completion achievement. Existing visible objects are never removed; coins and non-movement powerups continue; normal input resumes after landing. **Three TEMP preview defaults are committed in SOURCE, not in `main.tscn`** (they are plain vars — the scene cannot hold them): `BiomeDirector.debug_biome_seconds` `10.0`, `AuroraDirector.debug_aurora_interval_override` `10.0`, `AuroraDirector.debug_aurora_ignore_night` `true`; restore `0.0`, `0.0`, `false` before shipping. The preview interval also bypasses `MIN_RUN_TIME_SECONDS`. `aurora_borealis.md` |
