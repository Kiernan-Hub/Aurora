...
| 12 | **Aurora borealis** | The game's namesake. **Phase 1 (`AuroraDirector`, `SaveStore.aurora_count`) written 2026-09-07, NOT YET RUN — needs a project import (new `class_name`) then `check.sh`.** No visuals yet. ~60 min cumulative playtime, **night only** (blended `star_density` ≥ 0.8), one blend ramp, one aurora per run. **It must never arm terrain** — `arm_lake()` is the sole writer of `get_terrain_height`'s one runtime input. `aurora_borealis.md` |
