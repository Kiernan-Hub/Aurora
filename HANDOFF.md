# Handoff — background differentiation (moving off the Alto's-Adventure look)

Rewritten 2026-08-17 at the end of a session that **tried a direction, failed, and reverted
it**. The tree is back at a known-good background. The owner wants to keep going on the
background — read "Where to start" last, it is the actionable part.

Working agreement, still in force: **one numbered sub-step at a time, commit it alone, stop
and wait for an explicit "go."** Flag anything with real bug potential or a much-harder
alternative before building it, and let the owner choose. Don't self-verify visuals with
screenshots — the owner checks in-game.
...
**Option 4 from the original brainstorm has still never been tried, and it is the one the
owner picked.** Rework `get_ridge_height()` in `scripts/systems/background_generator.gd` to
produce jagged/faceted skylines instead of smooth sine humps. It drives all four live
`ParallaxLayer`s (`FarPeaks`, `FarRidge`, `MidRidge`, `PineLine`) and therefore all 8 biome
palettes, since `BiomeDirector` only recolours this shape.
...
- **`95d9ca4` is the revert**; the tree's background code is identical to `7ffd9f7`.
- Gates run and passing on the restored tree: `shipping_values_check`,
  `biome_schedule_check`, `terrain_invariant_check`. `project.godot` clean.
- **Moved out of shipping paths this session** (CLAUDE.md rule: reference art lives in
  `art_source/`, which is `.gdignore`d; the repo root and `assets/` both ship):
  `example.png` → `art_source/example.png` (it had a generated `.import` and a 1 MB `.ctex`,
  so it *was* being packed), and `assets/textures/background/raster_wall_1.png` /
  `raster_wall_2.png` → `art_source/` (unused by any scene, from the abandoned raster
  route). `assets/textures/background/` is now gone.
- `art_source/Glossy Frozen Lake.png` and `art_source/glossy biome trail.png` are the
  owner's own files, unrelated to this thread, deliberately left untracked.
